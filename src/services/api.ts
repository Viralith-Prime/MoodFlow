...
class ApiService {
  ...
  async getMoods(): Promise<ApiResponse<MoodEntry[]>> {
    try {
      ...
      const response = await fetch(`${API_BASE}/moods?userId=${this.userId}`);
      ...
      localStorage.setItem('moodflow-moods', JSON.stringify(moods));
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (result.mood.id !== newMood.id) {
              ...
              localStorage.setItem('moodflow-moods', JSON.stringify(updatedMoods));
  ...
  async getSettings(): Promise<ApiResponse<UserSettings>> {
    try {
      if (!this.isOnline) {
        // Fallback to localStorage when offline
        const localSettings = localStorage.getItem('moodflow-settings');
      ...
      // Update localStorage with server data
      localStorage.setItem('moodflow-settings', JSON.stringify(result.settings));
  ...
  async updateSettings(settingsData: Partial<UserSettings>): Promise<ApiResponse<UserSettings>> {
    try {
      ...
      const existingSettings = localStorage.getItem('moodflow-settings');
      const currentSettings = existingSettings ? JSON.parse(existingSettings) : {};
      const updatedSettings = { ...currentSettings, ...settingsData };
      localStorage.setItem('moodflow-settings', JSON.stringify(updatedSettings));
